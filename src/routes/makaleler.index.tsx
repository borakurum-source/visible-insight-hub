  ...
  return (
    <MotionPress className="block h-full">
      ...
      >
        ...
        <p className="mt-4 flex-1 text-sm leading-7 text-[#6B6A61]">{article.description}</p>
        <div className="mt-7 flex items-center justify-between border-t border-[#E3E0D5] pt-4 text-xs text-[#6B6A61]">
          <span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" />{article.readingTime}</span>
          <span className="flex items-center gap-1 font-semibold text-[#1B7F86]">
  ...
  return (
    <MarketingShell>
      <VisualHero
        ...
        title={<>Yapay zeka arama için <span className="text-[#3FBFB2]">ölçülebilir bilgi.</span></>}
      ...
      >
        <p className="text-sm text-slate-400">
          Öne çıkan rehber: <Link className="font-semibold text-[#3FBFB2] hover:text-white" to="/makaleler/$slug" params={{ slug: featured.slug }}>{featured.title}</Link>
        </p>
      </VisualHero>
      ...
      <section className="marketing-container px-4 py-16 md:px-6 md:py-24">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#1B7F86]">Rehberler</p>
            <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.04em] text-[#101211] md:text-4xl">Kavramı anlayın, sonra ölçün.</h2>
          </div>
          <Link to="/free-ai-readiness-report" className="text-sm font-bold text-[#1B7F86] hover:text-[#101211]">Ücretsiz raporu başlat →</Link>
        </div>
