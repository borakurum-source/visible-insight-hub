  ...
  return (
    <MotionPress className="block h-full">
      <Link
        ...
        className={`group flex h-full flex-col rounded-2xl border p-6 transition-[box-shadow,border-color] duration-200 motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#356AFF] focus-visible:ring-offset-2 ${featured ? "border-[#B7C8FF] bg-[#EEF3FF] shadow-[0_18px_42px_rgba(53,106,255,0.09)]" : "border-[#E6EAF2] bg-white hover:border-[#B7C8FF] hover:shadow-[0_14px_32px_rgba(11,16,32,0.06)]"}`}
      >
        <div className="flex items-center justify-between gap-3">
          <Badge variant="outline" className="border-[#DDE5F4] bg-white text-[#356AFF]">{article.category}</Badge>
        ...
        <div className="mt-7 flex items-center justify-between border-t border-[#DDE5F4] pt-4 text-xs text-[#667085]">
...
function ArticlesPage() {
  ...
  return (
    <MarketingShell>
        ...
        <div className="mt-8 flex flex-wrap gap-2" aria-label="Makale kategorileri">
          {categories.map((category) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-semibold transition-[background-color,color,transform] duration-150 active:scale-[0.98] motion-reduce:transition-none ${selectedCategory === category ? "border-[#356AFF] bg-[#356AFF] text-white" : "border-[#DDE5F4] bg-white text-[#667085] hover:border-[#B7C8FF] hover:text-[#0B1020]"}`}
            >
              {category}
            </button>
          ))}
      ...
      <section className="bg-[#F4F8FF] px-4 py-16 md:px-6">
        <div className="marketing-container flex flex-col items-start justify-between gap-6 rounded-2xl border border-[#DDE5F4] bg-white p-7 md:flex-row md:items-center">
          <div>
