...
function Hero() {
  return (
    <section id="olcum" className="visual-hero-surface relative isolate overflow-hidden border-b border-[#26302E] scroll-mt-16" data-testid="section-hero">
      ...
      <div className="pointer-events-none absolute -left-24 top-24 -z-10 h-64 w-64 rounded-full bg-[#35E1FF]/10 blur-3xl" aria-hidden="true" />
      <div className="marketing-container grid min-w-0 items-center gap-12 py-16 md:py-24 lg:grid-cols-[minmax(0,.92fr)_minmax(440px,1.08fr)] lg:gap-16 lg:py-28">
        <div className="space-y-7 text-white md:space-y-8">
          ...
          <div className="space-y-5">
            <h1 className="max-w-2xl text-4xl font-extrabold leading-[1.03] tracking-[-0.05em] text-white md:text-5xl lg:text-[60px]" data-testid="text-hero-headline">
              Yapay zeka cevaplarında markanız <span className="text-[#35E1FF]">kaynak</span> olarak seçiliyor mu?
            </h1>
          ...
          <div className="max-w-xl rounded-xl border border-white/15 bg-[#F7F9FC]/[0.08] p-4 backdrop-blur-md md:p-5" data-testid="hero-report-cta">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-white"><Target className="h-4 w-4 text-[#35E1FF]" /> İlk ölçümü başlatın</div>
        ...
        <Reveal className="relative" delay={0.06}>
          <div className="visual-panel-shadow relative overflow-hidden rounded-[28px] border border-white/15 bg-[#0B1020]" data-testid="hero-visual-panel">
            <div className="absolute inset-0 z-10 bg-gradient-to-t from-[#0B1020]/45 via-transparent to-transparent" aria-hidden="true" />
            <div className="absolute left-5 top-5 z-20 flex items-center gap-2 rounded-full border border-white/15 bg-[#0B1020]/70 px-3 py-1.5 backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-[#35E1FF]" /><span className="visual-source-label text-slate-300">SOURCE SIGNAL · 01</span></div>
...
function ProofStrip() {
  ...
  return (
    <section className="border-b border-[#E6EAF2] bg-[#F7F9FC]" data-testid="section-proof-strip">
      ...
      <div className="grid gap-10 lg:grid-cols-[.88fr_1.12fr] lg:gap-16">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {items.map(({ icon: Icon, title, body }, index) => (
            <Reveal key={title} delay={index * 0.06} className="rounded-xl border border-[#E6EAF2] bg-[#F7F9FC] p-5">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EDEFE9] text-[#356AFF]"><Icon className="h-5 w-5" /></div>
...
function KanıtMetaforuBandı() {
  return (
    <section className="border-y border-[#E6EAF2] bg-[#0B1020] py-16 md:py-24" data-testid="section-visual-evidence-gap">
      <div className="marketing-container grid items-center gap-10 lg:grid-cols-[1.08fr_.92fr] lg:gap-16">
        <Reveal className="relative overflow-hidden rounded-[26px] border border-white/15 bg-[#0B1020] visual-panel-shadow">
          ...
          <div className="absolute inset-x-5 bottom-5 flex items-center justify-between gap-4"><span className="visual-source-label text-[#35E1FF]">EVIDENCE GAP / 02</span><span className="font-mono text-[10px] text-slate-400">MISSING SOURCE</span></div>
        </Reveal>
        <div className="text-white">
          <p className="visual-source-label text-[#35E1FF]">GÖRÜNMEYEN KIRILMA</p>
...
function KaynakSinyalUygulama() {
  ...
  return (
    <section className="border-y border-[#E6EAF2] bg-[#F2F0E8] py-16 md:py-24" data-testid="section-source-signal-action">
      <div className="marketing-container">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.13em] text-[#356AFF]">Ürün akışı</p>
          <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.035em] text-[#0B1020] md:text-4xl">Kaynağı sinyale, sinyali doğru uygulamaya çevirin.</h2>
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {steps.map(({ number, icon: Icon, eyebrow, title, body }, index) => (
            <Reveal key={number} delay={index * 0.07} className="relative rounded-xl border border-[#E6EAF2] bg-[#F7F9FC] p-6 md:p-7">
              {index < 2 && <ArrowRight className="absolute -right-8 top-1/2 hidden h-5 w-5 text-[#356AFF] lg:block" aria-hidden="true" />}
              <div className="flex items-center justify-between"><span className="font-mono text-sm text-[#356AFF]">{number}</span><Icon className="h-5 w-5 text-[#57564E]" /></div>
              <p className="mt-8 text-xs font-semibold uppercase tracking-[0.12em] text-[#356AFF]">{eyebrow}</p>
              <h3 className="mt-3 text-xl font-bold text-[#0B1020]">{title}</h3>
...
function UrunAkisi() {
  ...
  return (
    <section className="border-y border-[#E6EAF2] bg-[#F7F9FC] py-16 md:py-24" data-testid="section-product-flow">
      <div className="marketing-container">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.13em] text-[#356AFF]">Ürünün içinden</p>
          <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.035em] text-[#0B1020] md:text-4xl">Ölçümden uygulamaya giden görünür bir karar zinciri.</h2>
          <p className="mt-4 text-base leading-7 text-[#667085]">Bunlar taslak görseller değil; OneCite ürünündeki gerçek ekranlardan alınan örneklerdir.</p>
        </div>
        <div className="mt-14 space-y-16 md:space-y-24">
          {rows.map((row, index) => (
            <Reveal key={row.step} className={`grid items-center gap-9 lg:grid-cols-2 lg:gap-16 ${index % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
              <div>
                <div className="flex items-center gap-3"><span className="font-mono text-xs text-[#356AFF]">{row.step}</span><span className="h-px w-8 bg-[#35E1FF]" /><span className="text-xs font-semibold uppercase tracking-[0.12em] text-[#667085]">{row.tag}</span></div>
                <h3 className="mt-5 text-2xl font-extrabold tracking-[-0.03em] text-[#0B1020] md:text-3xl">{row.title}</h3>
                <p className="mt-4 max-w-xl text-base leading-7 text-[#667085]">{row.body}</p>
              </div>
              <div className="overflow-hidden rounded-xl border border-[#E6EAF2] bg-[#EEF2F9]"><img src={row.img} alt={row.alt} className="h-auto w-full" /></div>
            </Reveal>
          ))}
        </div>
      </div>
...
function FilmFolkOrnegi() {
  ...
  return (
    <section className="marketing-container px-4 py-16 md:px-6 md:py-24" data-testid="section-case-study">
      <div className="overflow-hidden rounded-2xl border border-[#E6EAF2] bg-[#EDEFE9] p-7 md:p-12">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:gap-16">
          <div>
            <span className="inline-flex items-center rounded-full border border-[#C9C5B6] bg-[#F7F9FC] px-2.5 py-0.5 text-xs font-semibold text-[#356AFF]">Örnek ölçüm · FilmFolk</span>
            <h2 className="mt-5 text-3xl font-extrabold tracking-[-0.035em] text-[#0B1020] md:text-4xl">Bir vaka çalışması, yalnızca sonuç değil; sonuçtaki kanıt değişimidir.</h2>
            ...
            <Link to="/proof/filmfolk" className="mt-7 inline-flex items-center gap-2 text-sm font-bold text-[#0B1020] underline decoration-[#35E1FF] decoration-2 underline-offset-4 transition-colors hover:text-[#356AFF]">FilmFolk vaka çalışmasını inceleyin <ArrowRight className="h-4 w-4" /></Link>
          </div>
          <div className="grid grid-cols-2 gap-3 self-start">
            {metrics.map((metric) => (
              <div key={metric.label} className="rounded-xl border border-[#E6EAF2] bg-[#F7F9FC] p-4 md:p-5">
...
function PlanlaraBakis() {
  ...
  return (
    <section className="bg-[#EEF2F9] py-16 md:py-24" data-testid="section-plans-overview">
      <div className="marketing-container">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.13em] text-[#356AFF]">Planlar</p>
          <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.035em] text-[#0B1020] md:text-4xl">Ölçüm ritminize uygun bir başlangıç seçin.</h2>
          <p className="mt-4 text-base leading-7 text-[#667085]">Planları, limitleri ve ekibiniz için uygun çalışma alanını ayrı fiyatlandırma sayfasında net biçimde karşılaştırın.</p>
        </div>
        <div className="mt-10 grid gap-3 lg:grid-cols-3">
          {fits.map((fit) => (
            <Link to={fit.href} key={fit.title} className="group flex flex-col rounded-xl border border-[#E6EAF2] bg-[#F7F9FC] p-5 transition-transform hover:-translate-y-1">
              <p className="text-sm font-bold text-[#0B1020]">{fit.title}</p>
              <p className="mt-2 flex-1 text-sm leading-6 text-[#667085]">{fit.description}</p>
              <span className="mt-5 flex items-center gap-1 text-sm font-bold text-[#356AFF] group-hover:text-[#0B1020]">{fit.action} <ArrowRight className="h-3.5 w-3.5" /></span>
            </Link>
          ))}
        </div>
...
function SonCagri() {
  return (
    <section className="relative isolate overflow-hidden bg-[#0B1020] px-4 py-16 md:px-6 md:py-24" data-testid="section-final-cta">
      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-white/15 bg-[#0B1020]/70 px-7 py-12 text-center text-white backdrop-blur-md md:px-12">
