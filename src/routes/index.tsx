...
function Hero() {
  return (
    <section id="olcum" className="visual-hero-surface relative isolate overflow-hidden border-b border-[#1B2D52] scroll-mt-16" data-testid="section-hero">
      ...
      <div className="marketing-container grid min-w-0 items-center gap-12 py-16 md:py-24 lg:grid-cols-[minmax(0,.92fr)_minmax(440px,1.08fr)] lg:gap-16 lg:py-28">
        ...
        <Reveal className="relative" delay={0.06}>
          <div className="visual-panel-shadow relative overflow-hidden rounded-[28px] border border-white/15 bg-[#0B1020]" data-testid="hero-visual-panel">
            ...
            <div className="absolute left-5 top-5 z-20 flex items-center gap-2 rounded-full border border-white/15 bg-[#0B1020]/70 px-3 py-1.5 backdrop-blur"><span className="h-1.5 w-1.5 rounded-full bg-[#35E1FF] shadow-[0_0_12px_#35E1FF]" /><span className="visual-source-label text-slate-300">SOURCE SIGNAL · 01</span></div>
...
function ProofStrip() {
  ...
  return (
    <section className="border-b border-[#E6EAF2] bg-white" data-testid="section-proof-strip">
      ...
      <div className="grid gap-10 lg:grid-cols-[.88fr_1.12fr] lg:gap-16">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {items.map(({ icon: Icon, title, body }, index) => (
            <Reveal key={title} delay={index * 0.06} className="rounded-2xl border border-[#E6EAF2] bg-white p-5 shadow-[0_12px_28px_rgba(11,16,32,0.035)]">
...
function KaynakSinyalUygulama() {
  ...
  return (
    <section className="border-y border-[#E6EAF2] bg-[#F4F8FF] py-16 md:py-24" data-testid="section-source-signal-action">
      <div className="marketing-container">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-base leading-7 text-[#516078]">OneCite’in kapalı döngüsü: yapay zekanın kullanabileceği kanıtı kurar, seçimlerini ölçer ve en yüksek etkili iyileştirmeyi öne alır.</p>
        </div>
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {steps.map(({ number, icon: Icon, eyebrow, title, body }, index) => (
            <Reveal key={number} delay={index * 0.07} className="relative rounded-2xl border border-[#DDE5F4] bg-white p-6 md:p-7">
              {index < 2 && <ArrowRight className="absolute -right-8 top-1/2 hidden h-5 w-5 text-[#356AFF] lg:block" aria-hidden="true" />}
              <div className="flex items-center justify-between"><span className="font-mono text-sm text-[#356AFF]">{number}</span><Icon className="h-5 w-5 text-[#516078]" /></div>
...
function UrunAkisi() {
  ...
  return (
    <section className="border-y border-[#E6EAF2] bg-white py-16 md:py-24" data-testid="section-product-flow">
      <div className="marketing-container">
        <div className="max-w-2xl">
        ...
        <div className="mt-14 space-y-16 md:space-y-24">
          {rows.map((row, index) => (
            <Reveal key={row.step} className={`grid items-center gap-9 lg:grid-cols-2 lg:gap-16 ${index % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
              ...
              <div className="overflow-hidden rounded-2xl border border-[#E6EAF2] bg-[#F7F9FC] shadow-[0_18px_45px_rgba(11,16,32,0.08)]"><img src={row.img} alt={row.alt} className="h-auto w-full" /></div>
            </Reveal>
          ))}
        </div>
      </div>
    ...
    <section className="marketing-container px-4 py-16 md:px-6 md:py-24" data-testid="section-case-study">
      <div className="overflow-hidden rounded-3xl border border-[#DDE5F4] bg-[#EEF3FF] p-7 md:p-12">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:gap-16">
          <div>
            <span className="inline-flex items-center rounded-full border border-[#B7C8FF] bg-white px-2.5 py-0.5 text-xs font-semibold text-[#356AFF]">Örnek ölçüm · FilmFolk</span>
...
function PlanlaraBakis() {
  ...
  return (
    <section className="bg-[#F7F9FC] py-16 md:py-24" data-testid="section-plans-overview">
      <div className="marketing-container">
        <div className="max-w-2xl">
        ...
        <div className="mt-10 grid gap-3 lg:grid-cols-3">
          {fits.map((fit) => (
            <Link to={fit.href} key={fit.title} className="group flex flex-col rounded-2xl border border-[#E6EAF2] bg-white p-5 transition-transform hover:-translate-y-1">
