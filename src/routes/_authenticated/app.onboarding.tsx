  ...
  head: () => ({
    meta: [
      { title: "Kurulum — OneCite Paneli" },
      {
        name: "description",
        content:
          "Alan adınızı girin, marka kitabınızı onaylayın, promptları ve rakipleri seçin; ilk ölçümünüz otomatik başlasın.",
      },
      { property: "og:title", content: "Kurulum — OneCite Paneli" },
      {
        property: "og:description",
        content: "Üç adımda kaydedilebilir yapay zeka görünürlük kurulumu.",
      },
...
const ENGINES = [
  {
    key: "agent_web_grounded",
    name: "Kaynaklı web ölçümü",
    description: "Perplexity Agent ile güncel web kanıtı ve atıf ölçümü.",
  },
  {
    key: "model_panel",
    name: "Model paneli",
    description: "Yapılandırılmış model örneklemi; tüketici uygulaması ölçümü değildir.",
  },
] as const;
...
function OnboardingPage() {
  ...
  const loadOnboardingRun = useServerFn(getOnboardingRun);
  const persistOnboarding = useServerFn(saveOnboardingProgress);
  ...
  const [newCompetitor, setNewCompetitor] = useState<CompetitorEntry>({
    name: "",
    domain: "",
    type: "direct",
  });
  ...
  // 6. adım
  const [engines, setEngines] = useState<string[]>(["agent_web_grounded"]);
  ...
  const applyIntel = useCallback(
    (row: Record<string, unknown> | null | undefined, fallbackName: string) => {
      if (!row) return;
      setBook({
  ...
  useEffect(() => {
    if (!brandId) return;
    void Promise.all([
      loadOnboardingRun({ data: { brandId } }),
      loadIntel({ data: { brandId } }),
      listAllPrompts({ data: { brandId } }),
      loadCompetitors({ data: { brandId } }),
      loadEngines({ data: { brandId } }),
    ])
      .then(([run, intel, existingPrompts, existingCompetitors, existingEngines]) => {
        if (run && run.status !== "completed") {
          const progress = (run.progress ?? {}) as Record<string, unknown>;
          const phase = String(progress.phase ?? "");
          if (phase === "brand_review") setStep(3);
          else if (phase === "prompts") setStep(4);
          else if (phase === "competitors") setStep(5);
          else if (phase === "tracking") setStep(6);
          const sources = Array.isArray(progress.sources) ? progress.sources : [];
          setDiscoveredSources(sources as Array<{ title: string; url?: string }>);
        }
        applyIntel(intel as never, brand?.name ?? "");
        setPrompts(
          (existingPrompts ?? []).map((prompt) => ({
            id: prompt.id,
            text: prompt.text,
            funnelStage: normalizeFunnel(prompt.funnel_stage),
            status: prompt.status,
          })),
        );
        setCompetitors(existingCompetitors?.competitors ?? []);
        setEngines(
          existingEngines?.engines?.length ? existingEngines.engines : ["agent_web_grounded"],
        );
      })
      .catch(() => undefined);
  }, [
  ...
  async function startScan() {
    ...
    if (!clean) {
      toast.error("Geçerli bir alan adı girin, örn. markaniz.com");
      return;
    }
    setBusy(true);
    setStep(2);
    ...
    try {
      ...
      await persistOnboarding({
        data: {
          ...
          currentStep: 1,
          status: "scanning",
          market: { domain: clean, language },
          progress: { phase: "scan", percent: 15 },
        },
      });
      ...
      setDiscoveredSources(suggested);
      if (suggested.length) {
        await addSources({ data: { brandId: created.id, items: suggested.slice(0, 8) } }).catch(
          () => undefined,
        );
      }
      await persistOnboarding({
        data: {
          ...
          market: { domain: clean, language },
          discoveredFacts: Object.entries(intel as Record<string, unknown>).map(([key, value]) => ({
            key,
            value,
            status: "external",
          })),
          progress: { phase: "brand_review", percent: 100, sources: suggested },
        },
      });
      setStep(3);
    } catch (error) {
    ...
    try {
      ...
      await persistOnboarding({
        data: {
          ...
          currentStep: 3,
          status: "awaiting_approval",
          proposedPrompts: rows,
          progress: { phase: "prompts", percent: 35, sources: discoveredSources },
        },
      });
      setStep(4);
    } catch (error) {
  ...
  async function savePromptsAndLoadCompetitors() {
    ...
    try {
      await savePromptEdits({
        data: {
          items: prompts.map((p) => ({ id: p.id, text: p.text, funnelStage: p.funnelStage })),
        },
      });
      ...
      await persistOnboarding({
        data: {
          ...
          proposedCompetitors: result.competitors,
          progress: { phase: "competitors", percent: 65, sources: discoveredSources },
        },
      });
      setStep(5);
    } catch (error) {
  ...
  async function saveCompetitorsAndLoadEngines() {
    ...
    try {
      ...
      const current = await loadEngines({ data: { brandId } }).catch(() => ({
        engines: ["agent_web_grounded"],
      }));
      ...
      await persistOnboarding({
        data: {
          ...
          status: "awaiting_approval",
          proposedPrompts: prompts,
          proposedCompetitors: competitors,
          progress: { phase: "tracking", percent: 85, sources: discoveredSources },
        },
      });
      setStep(6);
    } catch (error) {
    ...
    try {
      ...
      await persistOnboarding({
        data: {
          ...
          currentStep: 3,
          status: "completed",
          progress: { phase: "completed", percent: 100, sources: discoveredSources },
        },
      });
      await refreshSession();
  ...
  if (step === 2) {
    return (
      <WizardFrame
        step={2}
        total={TOTAL_STEPS}
        title="Marka Hafızası hazırlanıyor"
        subtitle="Hızlı ana sayfa ve URL haritası önce gelir; kapsamlı tarama arka planda ilerler."
      >
        <Card>
    ...
    return (
      ...
      >
        <div className="flex gap-2 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
          ...
          <p>
            Aşağıdaki bilgiler markanızı doğru yansıtmıyorsa düzeltin. Sitenizde bot korumasi (örn.
            Cloudflare) varsa içeriğe tam erişemeyip eksik çıkarım yapmış olabiliriz.
          </p>
        </div>
        {discoveredSources.length ? (
          <div className="space-y-2 rounded-lg border border-border p-3">
            <p className="text-xs font-semibold">Bulunan kaynaklar</p>
            <div className="flex flex-wrap gap-2">
              {discoveredSources.slice(0, 12).map((source) =>
                source.url ? (
                  <a
                    key={source.url}
                    href={source.url}
            ...
            <p className="text-[11px] text-muted-foreground">
              Harici bulgular siz onaylamadan marka gerçeğine dönüşmez.
            </p>
          </div>
        ) : null}
        <div className="grid gap-4 sm:grid-cols-2">
          <Field
            label="Marka adı"
            value={book.brandName}
            onChange={(v) => setBook({ ...book, brandName: v })}
          />
          <Field
            label="Sektör"
            value={book.industry}
            onChange={(v) => setBook({ ...book, industry: v })}
          />
          <div className="space-y-1.5">
            <Label>Birincil dil</Label>
            <Select
              value={book.language || "Türkçe"}
              onValueChange={(v) => setBook({ ...book, language: v })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <Field
            label="Ana lokasyon"
            value={book.location}
            onChange={(v) => setBook({ ...book, location: v })}
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            rows={2}
            value={book.keyFeatures}
            onChange={(e) => setBook({ ...book, keyFeatures: e.target.value })}
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            rows={5}
            value={book.detailedDescription}
            onChange={(e) => setBook({ ...book, detailedDescription: e.target.value })}
          />
        </div>
      </WizardFrame>
    );
    ...
    return (
      <WizardFrame
        ...
        footer={
          <>
            ...
            <Button
              onClick={savePromptsAndLoadCompetitors}
              disabled={busy || prompts.length === 0 || promptQuotaExceeded}
            >
              {busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Devam et <ArrowRight className="ml-1.5 h-4 w-4" />
            </Button>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
          <span>
            {prompts.length} prompt · Üst {funnelCounts.top} · Orta {funnelCounts.middle} · Alt{" "}
            {funnelCounts.bottom}
          </span>
          {maxPrompts > 0 ? (
        ...
        {promptQuotaExceeded ? (
          <p className="rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
            Planınızın prompt hakkını aştınız. Devam etmek için fazla promptları silin veya
            planınızı yükseltin.
          </p>
        ) : null}
        ...
        <div className="space-y-2">
          ...
          {prompts.length === 0 ? (
            <p className="p-3 text-sm text-muted-foreground">
              Henüz prompt yok, aşağıdan ekleyebilirsiniz.
            </p>
          ) : null}
        </div>
        ...
        <div className="space-y-2 rounded-lg border border-dashed border-border p-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            />
            <Select
              value={newPromptStage}
              onValueChange={(v) => setNewPromptStage(normalizeFunnel(v))}
            >
              <SelectTrigger className="sm:w-48">
                <SelectValue />
              </SelectTrigger>
      ...
      >
        {maxCompetitors > 0 ? (
          <p className="text-xs text-muted-foreground">
            Planınızda {maxCompetitors} rakip takip edebilirsiniz · {competitors.length} seçili
          </p>
        ) : null}
        <div className="space-y-2">
          ...
          {competitors.length === 0 ? (
            <p className="p-3 text-sm text-muted-foreground">Henüz rakip yok, aşağıdan ekleyin.</p>
          ) : null}
        </div>
        ...
        <div className="space-y-2 rounded-lg border border-dashed border-border p-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setNewCompetitor({ ...newCompetitor, type: v as CompetitorType })
              }
            >
              <SelectTrigger className="sm:w-44">
                <SelectValue />
            ...
            <Button
              ...
              onClick={() => {
                ...
                setCompetitors([
                  ...competitors,
                  { name: name || dom, domain: dom, type: newCompetitor.type },
                ]);
          ...
          {competitorQuotaFull ? (
            <p className="text-xs text-destructive">
              Plan limitiniz dolu. Yeni rakip eklemek için bir rakibi kaldırın veya planınızı
              yükseltin.
            </p>
          ) : null}
        </div>
  ...
  return (
    ...
    >
      <p className="text-center text-sm text-muted-foreground">
        {engines.length} / {ENGINES.length} motor izleniyor
      </p>
        ...
        {ENGINES.map((engine) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                setEngines(
                  active ? engines.filter((e) => e !== engine.key) : [...engines, engine.key],
                )
              }
      ...
      <p className="text-center text-xs text-muted-foreground">
        Kurulumu tamamladığınızda ilk ölçümünüz otomatik başlar ve skorunuz, seçilen kaynaklarnız ve
        ilk görevleriniz oluşur.
      </p>
    </WizardFrame>
  );
}
