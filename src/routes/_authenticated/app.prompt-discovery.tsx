  ...
  head: () => ({
    meta: [
      { title: "Prompt Keşfi — OneCite Paneli" },
      {
        name: "description",
        content:
          "Marka adı geçmeyen ama AI asistanlarının markanızı önerebileceği fırsat prompt adaylarını keşfedin.",
      },
      { property: "og:title", content: "Prompt Keşfi — OneCite Paneli" },
      {
        property: "og:description",
        content: "GEO reverse-engineering ile fırsat prompt adayları.",
      },
  ...
  const add = useMutation({
    mutationFn: (item: DiscoveredPrompt) =>
      addPrompts({
        data: {
          brandId: brand!.id,
          items: [{ text: item.text, cluster: item.cluster, intent: item.intent }],
        },
      }),
    ...
    <>
      ...
      <PanelPageHeading
        meta={{
          title: "Prompt Keşfi",
          description:
            "Marka adı hiç geçmeyen ama bir AI asistanının markanızı önerebileceği fırsat sorularını keşfedin.",
          icon: Compass,
        }}
        action={
          <Button
        ...
        <CardContent className="flex flex-col items-start gap-4 p-6 md:flex-row">
          ...
          <div className="flex-1 space-y-2">
            ...
            <p className="max-w-2xl text-sm text-muted-foreground">
              Mevcut Marka Zekası özetinden yola çıkarak, markanız hiç geçmeyen ama AI
              asistanlarının markanızı önerebileceği soruları üretir. Onayladığınız adaylar
              Promptlar sayfasına taşınır ve izlemeye başlar. Liste Fırsat skoruna göre sıralanır.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        {isFetching && visible.length === 0 ? (
          <Card>
            <CardContent className="flex items-center gap-2 p-6 text-sm text-muted-foreground">
              <Loader2 className="h-4 w-4 animate-spin" /> Fırsat soruları hazırlanıyor…
            </CardContent>
          </Card>
        ) : null}
        {!isFetching && visible.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-sm text-muted-foreground">
              Bu sayfa kendiliğinden çalışmaz. “Aday üret” dediğinizde marka zekanızdan yeni fırsat
              soruları çıkarılır.
            </CardContent>
          </Card>
        ) : null}
          ...
          <Card key={c.text}>
            <CardContent className="flex flex-col gap-3 p-4">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <Badge
                  variant="outline"
                  className={`gap-1 text-[10px] ${opportunityTone(c.opportunityScore)}`}
                >
              ...
              <div className="flex items-center justify-end gap-2">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => setDismissed((prev) => [...prev, c.text])}
                >
