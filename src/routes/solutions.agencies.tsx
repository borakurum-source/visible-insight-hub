      ...
      <section className="marketing-container px-4 py-16 md:px-6 md:py-24">
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {workflow.map(({ icon: Icon, step, title, body }) => (
            <article key={step} className="rounded-2xl border border-[#E6EAF2] bg-white p-6">
          ...
          <div className="rounded-2xl bg-[#F7F9FC] p-6">
            ...
            <div className="mt-6 space-y-3">
              {outcomes.map((outcome) => (
                <div key={outcome} className="flex items-center gap-3 rounded-xl border border-[#E6EAF2] bg-white p-4 text-sm font-semibold text-[#0B1020]"><FileOutput className="h-4 w-4 text-[#356AFF]" />{outcome}</div>
              ))}
            </div>
          </div>
        </div>
