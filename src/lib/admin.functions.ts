...
export const adminOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [profiles, subs, errors, usage, batches, recentUsers] = await Promise.all([
      ...
      supabaseAdmin
        .from("error_logs")
        .select("id, level, message, path, created_at, resolved")
    ...
    return {
      totals: {
        ...
        trialing: rows.filter(
          (r) => (r.plan === "trial" || r.plan === "free") && (r.trial_ends_at ?? "") > nowIso,
        ).length,
        trialExpired: rows.filter(
          (r) => (r.plan === "trial" || r.plan === "free") && (r.trial_ends_at ?? "") <= nowIso,
        ).length,
        paying: (subs.data ?? []).filter((s) => s.status === "active" || s.status === "trialing")
          .length,
  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      supabaseAdmin
        .from("profiles")
        .select(
          "id, email, full_name, plan, plan_source, plan_expires_at, trial_ends_at, suspended, created_at",
        )
        .order("created_at", { ascending: false }),
      ...
      supabaseAdmin
        .from("api_usage_log")
        .select("user_id, brand_id, cost_usd, created_at")
        .gte("created_at", monthAgo),
    ]);
    const brandCount = new Map<string, number>();
    for (const m of members ?? []) brandCount.set(m.user_id, (brandCount.get(m.user_id) ?? 0) + 1);
    for (const b of brands ?? [])
      if (!brandCount.has(b.created_by)) brandCount.set(b.created_by, 1);
    ...
    for (const m of members ?? [])
      if (!brandOwner.has(m.brand_id)) brandOwner.set(m.brand_id, m.user_id);
...
export const adminCustomerDetail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [brands, prompts, competitors, content, subs, notes, errors, usage] = await Promise.all([
      ...
      brandIds.length
        ? supabaseAdmin.from("prompts").select("id, status").in("brand_id", brandIds)
        : Promise.resolve({ data: [] as any[] }),
      brandIds.length
        ? supabaseAdmin.from("competitor_candidates").select("id, status").in("brand_id", brandIds)
        : Promise.resolve({ data: [] as any[] }),
      brandIds.length
        ? supabaseAdmin.from("content_drafts").select("id").in("brand_id", brandIds)
        : Promise.resolve({ data: [] as any[] }),
      supabaseAdmin
        .from("subscriptions")
        .select("*")
      ...
      supabaseAdmin
        .from("api_usage_log")
        .select("provider, cost_usd, created_at, brand_id, user_id")
        .or(
          brandIds.length
            ? `user_id.eq.${data.userId},brand_id.in.(${brandIds.join(",")})`
    ...
    const integrations = brandIds.length
      ? ((
          await supabaseAdmin
            ...
            .select("brand_id, provider, status, last_sync_at")
    ...
    return {
      ...
      integrations,
      usage: {
        prompts: (prompts.data ?? []).filter(
          (p: any) => p.status === "approved" || p.status === "tracked",
        ).length,
...
export const adminSetPlan = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (input: { userId: string; plan: string; expiresAt?: string | null; reason?: string }) => input,
  )
  .handler(async ({ data, context }) => {
    ...
    await audit(
      ...
      { plan: data.plan, expiresAt: data.expiresAt ?? null, reason: data.reason ?? null },
    );
    return { ok: true };
  });
...
export const adminExtendTrial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: profile } = await session.supabaseAdmin
      .from("profiles")
    ...
    const { error } = await session.supabaseAdmin
      .from("profiles")
      .update({ trial_ends_at: next, plan: "trial", plan_source: "manual" })
      .eq("id", data.userId);
    if (error) throw new Error(error.message);
...
export const adminToggleSuspend = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await session.supabaseAdmin
      ...
      .eq("id", data.userId);
    if (error) throw new Error(error.message);
    await audit(session, data.suspended ? "account.suspend" : "account.unsuspend", {
      type: "user",
      id: data.userId,
    });
    return { ok: true };
...
export const adminDeleteUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: owned } = await session.supabaseAdmin
      .from("brands")
      .select("id")
...
export const adminAddNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await session.supabaseAdmin
      .from("admin_notes")
      .insert({ user_id: data.userId, admin_id: session.adminId, note });
    if (error) throw new Error(error.message);
...
export const adminListSubscriptions = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (subs ?? []).map((s) => ({
      ...
      profilePlan: map.get(s.user_id)?.plan ?? null,
    }));
  });
...
export const adminApiUsage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows } = await supabaseAdmin
      .from("api_usage_log")
      .select(
        "provider, operation, model, brand_id, user_id, duration_ms, input_tokens, output_tokens, cost_usd, cached, status, error, created_at",
      )
    ...
    return {
      ...
      brands: Array.from(byBrand.values())
        .sort((a, b) => b.cost - a.cost)
        .slice(0, 15),
      customers,
...
export const adminProviderStatus = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const providers = [
      {
        key: "perplexity-agent",
        label: "Perplexity Agent",
        env: "PERPLEXITY_API_KEY",
        usage: "Kaynaklı araştırma, ölçüm ve güçlü model yönlendirme",
      },
      {
        key: "perplexity-router",
        label: "Perplexity Router",
        env: "PERPLEXITY_API_KEY",
        usage: "Hızlı toplu analiz; erişim yoksa Agent Luna fallback",
      },
      {
        key: "lovable-ai",
        label: "Lovable AI",
        env: "LOVABLE_API_KEY",
        usage: "E-posta ve yardımcı modeller",
      },
      {
        key: "firecrawl",
        label: "Firecrawl",
        env: "FIRECRAWL_API_KEY",
        usage: "Site tarama / render",
      },
      {
        key: "google",
        label: "Google (GSC & GA4)",
        env: "GOOGLE_OAUTH_CLIENT_ID",
        usage: "Müşteri bazlı trafik verisi",
      },
    ...
    return providers.map((p) => {
      ...
      return {
        ...
        errors7d: errors,
        lastCall,
        health: !process.env[p.env]
          ? "missing"
          : errors > own.length * 0.2 && own.length > 0
            ? "degraded"
            : "ok",
      };
    });
...
export const adminListErrors = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (input: { level?: string; resolved?: boolean; search?: string; source?: string }) => input,
  )
  .handler(async ({ data, context }) => {
    const { assertAdmin } = await import("./admin.server");
    const { supabaseAdmin } = await assertAdmin(context);
...
export const adminSaveEmailTemplate = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: existing } = await session.supabaseAdmin
...
export const adminBroadcast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const person of list) {
      const ok = await sendRawEmail(person.email!, data.subject, data.body, {
...
export const adminUserEmailAction = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (input: {
      userId: string;
      action: "reset_password" | "resend_verification" | "change_email";
      newEmail?: string;
    }) => input,
  )
  .handler(async ({ data, context }) => {
    ...
    if (data.action === "change_email") {
      ...
      const { error } = await session.supabaseAdmin.auth.admin.updateUserById(data.userId, {
        email: next,
        email_confirm: true,
      });
      if (error) throw new Error(error.message);
    ...
    const { data: link, error } = await session.supabaseAdmin.auth.admin.generateLink({
      type: type as any,
      email,
    });
    if (error) throw new Error(error.message);
...
export const adminListPosts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabaseAdmin
      .from("blog_posts")
      .select(
        "id, slug, title, description, category, status, published_at, updated_at, read_minutes, cover_image_url",
      )
...
export const adminSavePost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await audit(
      session,
      data.id ? "blog.update" : "blog.create",
      { type: "blog_post", id: saved.id },
