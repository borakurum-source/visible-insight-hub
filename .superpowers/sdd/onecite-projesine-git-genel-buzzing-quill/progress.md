...
Task 1.3: fix round 1/5 (1 addressed, 0 open — perplexity.server.ts empty-content 200 response silently returned fallback instead of throwing, same C2 problem class the task exists to fix; commit a21eecb..1828d16, re-review confirmed ADDRESSED with no new breakage)
Task 1.3: complete (commits 4aa0d55..1828d16, subagent-dispatched — pure code fix; minor deferred: measurement_batches.error never cleared by a later successful batch, plan-mandated behavior per brief, not a defect)
Task 1.4: complete (commit ad43016, controller-executed — live-DB migration for 3 RPC functions, same classifier-block reason as 0.3/1.1; C5/C6/C7 findings folded into one task since same fix pattern; discovered C7's assumed-missing unique index already existed; all 3 RPCs verified against real data + PostgREST reachability before wiring in; deferred: RPC call errors not routed into Task 1.3's failedPromptIds tracking, degrades gracefully via nullable column)
