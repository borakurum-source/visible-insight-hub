...
REVOKE SELECT ON onecite.blog_posts FROM anon;
GRANT SELECT (id, slug, title, description, category, tags, cover_image_url, og_image_url, canonical_url, body, answer_summary, faq, sources, read_minutes, status, author, published_at, created_at, updated_at) ON onecite.blog_posts TO anon;
...
ALTER TABLE onecite.reports DROP COLUMN IF EXISTS is_public;
...
USING (bucket_id = 'blog-media' AND onecite.has_role(auth.uid(), 'admin'))
WITH CHECK (bucket_id = 'blog-media' AND onecite.has_role(auth.uid(), 'admin'));